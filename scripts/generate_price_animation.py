...
K = 5  # sous-frames interpolees entre trimestres
NF = (len(Q) - 1) * K + 1
def price_at(w, pos):
...
def draw(frame):
    ...
    for w, pls in ward_patches.items():
        ...
        for p in pls:
            p.set_facecolor(c)
    qtext.set_text(Q[qi])
    citytext.set_text(f"citywide median  {int(sorted(vals)[len(vals)//2])}k / m2")
    return []

anim = animation.FuncAnimation(fig, draw, frames=NF, interval=90, blit=False)
...
# export MP4 (+ GIF)
try:
    anim.save(str(OUT_MP4), writer=animation.FFMpegWriter(fps=14, bitrate=2400),
              savefig_kwargs={"facecolor": fig.get_facecolor()})
    print("MP4 ecrit:", OUT_MP4.name, OUT_MP4.stat().st_size // 1024, "ko")
except Exception as e:
    print("MP4 echec:", str(e)[:150])
try:
    anim.save(str(OUT_GIF), writer=animation.PillowWriter(fps=12),
              savefig_kwargs={"facecolor": fig.get_facecolor()})
