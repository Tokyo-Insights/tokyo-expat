  ...
  return (
    <header className="bg-[#0f2744] text-white sticky top-0 z-50 shadow-lg">
      <div className="max-w-6xl mx-auto px-4 py-4 flex items-center justify-between">
        ...
        {/* Desktop nav (Accueil = le logo, Contact = le bouton CTA -> retires pour alleger) */}
        <nav className="hidden lg:flex items-center gap-5 text-sm font-medium">
          <Link href={`/${locale}/services`} className="hover:text-[#e84141] transition-colors whitespace-nowrap">{nav.services}</Link>
          <Link href={`/${locale}/listings`} className="hover:text-[#e84141] transition-colors whitespace-nowrap">{nav.listings}</Link>
          <Link href={`/${locale}/data`} className="hover:text-[#e84141] transition-colors whitespace-nowrap">{nav.data}</Link>
          <Link href={`/${locale}/blog`} className="hover:text-[#e84141] transition-colors whitespace-nowrap">{nav.blog}</Link>
          <Link href={`/${locale}/about`} className="hover:text-[#e84141] transition-colors whitespace-nowrap">{nav.about}</Link>
          <Link href={`/${locale}/checklist`} className="border border-[#e84141]/60 text-[#e84141] hover:bg-[#e84141] hover:text-white px-3 py-1.5 rounded-lg transition-colors whitespace-nowrap">{nav.checklist}</Link>
      ...
      {/* Mobile menu */}
      {open && (
        <div className="lg:hidden bg-[#0f2744] border-t border-gray-700 px-4 py-4 flex flex-col gap-4 text-sm">
          <Link href={`/${locale}/services`} onClick={() => setOpen(false)}>{nav.services}</Link>
          <Link href={`/${locale}/listings`} onClick={() => setOpen(false)}>{nav.listings}</Link>
